...
- `POST /api/start` - стартует сессию, возвращает UUID-ключ (используется дальше в заголовке `Key`).
  - Тело запроса: строка с шестизначным PIN-кодом
  - Ошибки:
    - `400` - PIN в неверном формате
    - `401` - PIN неверный
...
- `GET /api/current_question` - возвращает текст текущего вопроса для активной сессии.
  - Заголовок: `Key: <uuid>`
  - Ошибки:
    - `404` - вопросы закончились
...
- `POST /api/admin/reset_session` - сбрасывает сессию и текущий индекс вопроса, генерирует и возвращает новый шестизначный PIN-код для `/api/start`.
